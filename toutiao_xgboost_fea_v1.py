...
class Evaluator:
    ...
    def words_tfidf(self):
        ...
        self.user_words_tfidf_feas = [(get_mean(x), get_max(x), get_min(x)) for
                                      x in
                                      user_words_tfidf]
    ...
    def characters_tfidf(self):
        ...
        question_characters_tfidf = self.character_tfidf[
        ...
        # (0:mean, 1:max, 2:min)
        self.question_characters_tfidf_feas = [
            (get_mean(x), get_max(x), get_min(x))
            for x in
    ...
    def deal_questions_feas(self):
        ...
        self.question_info['characters_num'] = [len(x) for x in
                                                self.question_info[
                                                    'character_seq']]
    ...
    def deal_user_feas(self):
        ...
        self.user_info['reply_num'] = self.user_info['user_id'].apply(
            get_user_reply)
    ...
    def deal_question_user_feas(self):
        ...
        self.train_set['tag_w2v_sim'] = \
            [self.tag_w2v.n_similarity(
                    [str(self.question_info['question_tag'][test[
                        0]])], self.user_info['user_tags'][test[1]].split('/'))
             for test in self.train_set.values]
        # remove nan
        self.train_set['tag_w2v_sim'] = [x if type(x) == np.float64 else 0.0 for
                                         x
                                         in \
                                         self.train_set['tag_w2v_sim']]
        ...
        # remove nan
        self.valid_set['tag_w2v_sim'] = [x if type(x) == np.float64 else 0.0 for
                                         x
                                         in \
                                         self.valid_set['tag_w2v_sim']]
        ...
        self.train_set['words_w2v_sim'] = \
            [self.word_w2v.n_similarity(
                    np.array(self.question_info['words_seq'][test[0]],
                             dtype=str),
                    np.array(self.user_info['user_desc_words_sec'][test[1]],
                             dtype=str))
                for test in self.train_set.values]
        self.train_set['words_w2v_sim'] = [x if type(x) == np.float64 else 0.0
                                           for x
                                           in \
                                           self.train_set['words_w2v_sim']]
        ...
        self.valid_set['words_w2v_sim'] = \
            [self.word_w2v.n_similarity(
                    ...
                    np.array(self.user_info['user_desc_words_sec'][test[1]],
                             dtype=str))
                for test in self.valid_set.values]
        self.valid_set['words_w2v_sim'] = [x if type(x) == np.float64 else 0.0
                                           for x
                                           in \
                                           self.valid_set['words_w2v_sim']]
        ...
        self.train_set['characters_w2v_sim'] = \
            [self.character_w2v.n_similarity(
                    np.array(self.question_info['character_seq'][test[0]],
                             dtype=str),
                    np.array(
                            self.user_info['user_desc_characters_sec'][test[1]],
                            dtype=str))
                for test in self.train_set.values]
        self.train_set['characters_w2v_sim'] = [
        ...
        user_features = {
            ...
            'user_characters_num': lambda x: self.user_info['characters_num'][
                x],
        ...
        question_features = {
            ...
            'question_good_reply_num': lambda x:
            self.question_info['good_reply_num'][x],
            'question_words_num': lambda x: self.question_info['words_num'][x],
            'question_characters_num': lambda x:
            self.question_info['characters_num'][x],
            'question_words_tfidf_mean': lambda x:
            self.question_info['words_tfidf_mean'][x],
            'question_words_tfidf_max': lambda x:
            self.question_info['words_tfidf_max'][x],
            'question_words_tfidf_min': lambda x:
            self.question_info['words_tfidf_min'][x],
        ...
        self.submit_valid_set['characters_w2v_sim'] = [
            ...
            self.submit_valid_set[
                'characters_w2v_sim']]
        # for key, func in question_features.items():
        #   self.submit_valid_set[key] = self.submit_valid_set['qid'].apply(
        # func)
        
        for key, func in user_features.items():
            self.submit_valid_set[key] = self.submit_valid_set['uid'].apply(
                func)
    ...
    def deal_v2_gesd_aesd_sim_fetures(self):
        ...
        self.submit_valid_set = pd.merge(self.submit_valid_set, gesd_submit_sim,
                                         how='inner',
                                         on=['qid', 'uid'])
    ...
    def building_features(self):
        self.log(
            'start building features version %d' % self.conf.get('version'))
        self.words_tfidf()
    ...
    def save_features(self):
        ...
        pickle.dump(self.train_set_group,
                    open('features/%s/train_set_group.pkl' %
                         self.conf.get('version'), 'wb'))
        pickle.dump(self.valid_set, open('features/%s/valid_set.pkl' %
                                         self.conf.get('version'), 'wb'))
        pickle.dump(self.valid_set_group,
                    open('features/%s/valid_set_group.pkl' %
                         self.conf.get('version'), 'wb'))
        pickle.dump(self.submit_valid_set,
                    open('features/%s/submit_valid_set.pkl' %
                         self.conf.get('version'), 'wb'))
    ...
    def load_features(self):
        if os.path.exists('features/%s/' % self.conf.get('version')):
            ...
            self.train_set_group = self.load_feature(
                'features/%s/train_set_group.pkl' %
                self.conf.get('version'))
            self.valid_set = self.load_feature('features/%s/valid_set.pkl' %
                                               self.conf.get('version'))
            self.valid_set_group = self.load_feature(
                'features/%s/valid_set_group.pkl' %
                self.conf.get('version'))
            self.submit_valid_set = self.load_feature(
                'features/%s/submit_valid_set.pkl' %
                self.conf.get('version'))
    ...
    def train(self):
        ...
        train = self.train_set.drop(
                [x for x in self.train_set.keys() if
                 not self.conf['features'].has_key(x)],
                axis=1)
        valid = self.valid_set.drop(
                [x for x in self.valid_set.keys() if
                 not self.conf['features'].has_key(x)]
                ,
                axis=1)
        ...
        self.bst = xgb.train(self.conf['xgb_param'], dtrain, nb_epoch,
                             watchlist,
    ...
    def train_with_total(self, iteratoion=None):
        ...
        # valid = self.valid_set.drop(['question_id', 'user_id', 'answer_flag',
        #                              'is_valid'], axis=1)
        train = train_total.drop(
                [x for x in train_total.keys() if
                 not self.conf['features'].has_key(x)]
                ,
                axis=1)
        ...
        self.bst = xgb.train(self.conf['xgb_param'], dtrain, best_iteration,
                             watchlist,
                             feval=self.ndcgerror)
    ...
    def predict(self):
        ...
        online_valid = self.submit_valid_set.drop(
                [x for x in self.submit_valid_set.keys() if not
                self.conf['features'].has_key(x)]
                , axis=1)
...
if __name__ == '__main__':
    ...
    conf = {
        ...
        'version': version,
        'features': {
            'characters_w2v_sim': 532,
            'question_characters_num': 117,  # low
            'question_characters_tfidf_max': 413,
            'question_characters_tfidf_mean': 309,
            'question_characters_tfidf_min': 282,
            'question_good_reply_num': 333,
            'question_point_num': 538,  # low
            'question_reply_num': 350,
            'question_words_num': 108,
            'question_words_tfidf_max': 406,
            'question_words_tfidf_mean': 359,
            'question_words_tfidf_min': 364,
            'tag_w2v_sim': 1437,
            'user_characters_num': 242,
            'user_characters_tfidf_max': 340,
            'user_characters_tfidf_mean': 321,
            'user_characters_tfidf_min': 253,
            'user_ignore_num': 873,
            'user_ignore_rate': 795,
            'user_reply_num': 558,
            'user_reply_rate': 1732,
            'user_tag_num': 233,
            'user_total_num': 794,
            'user_words_num': 164,
            'words_w2v_sim': 427,
