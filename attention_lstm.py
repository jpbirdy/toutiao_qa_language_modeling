...
class AttentionLSTM(LSTM):
    def __init__(self, output_dim, attention_vec, attn_activation='tanh',
        ...
        self.n_attention_dim = output_dim if n_attention_dim is None else \
          n_attention_dim
    ...
    def build(self, input_shape):
        ...
        if hasattr(self.attention_vec, '_keras_shape'):
            ...
            raise Exception(
                'Layer could not be build: No information about expected '
                'input shape.')
        ...
        self.trainable_weights += [self.U_a, self.U_m, self.U_s, self.b_a,
                                   self.b_m, self.b_s]
    ...
    def step(self, x, states):
        ...
        # Intuitively it makes more sense to use a sigmoid (was getting some
        # NaN problems
        # which I think might have been caused by the exponential function ->
        #  gradients blow up)
