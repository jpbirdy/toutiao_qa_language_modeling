...
from keras.layers import merge, Embedding, Dropout, Convolution1D, Lambda, \
  LSTM, \
    Dense, TimeDistributed, constraints
...
class LanguageModel:
    ...
    def get_answer(self):
        if self._answer is None:
            self._answer = Input(shape=(self.config['answer_len'],),
                                 dtype='int32', name='answer')
        return self._answer
    ...
    def get_similarity(self):
        ''' Specify similarity in configuration under 'similarity_params' ->
        'mode'
        If a parameter is needed for the model, specify it in
        'similarity_params'
        ...
        l2_norm = lambda a, b: K.sqrt(
            K.sum((a - b) ** 2, axis=1, keepdims=True))
        ...
        if similarity == 'cosine':
            ...
            return lambda x: (
                             params['gamma'] * dot(x[0], x[1]) + params['c'])\
                             ** \
                             params['d']
        elif similarity == 'sigmoid':
            return lambda x: K.tanh(
                params['gamma'] * dot(x[0], x[1]) + params['c'])
        elif similarity == 'rbf':
            return lambda x: K.exp(
                -1 * params['gamma'] * l2_norm(x[0], x[1]) ** 2)
    ...
    def get_qa_model(self):
        ...
        if self._qa_model is None:
            ...
            dropout = Dropout(
                self.similarity_params.get('similarity_dropout', 0.2))
    ...
    def get_train_model(self):
        ...
        if self._qa_model is None:
            ...
            #         self.relu(0.9 - self.get_similarity()(x),
            # max_value=1.), 1.)
            similarity = lambda x: K.expand_dims(self.get_similarity()(x), 1)
            qa_model = merge([dropout(question_output), dropout(answer_output)],
                             mode=similarity, output_shape=lambda _: (None, 1))
    ...
    def fit(self, x, **kwargs):
        assert self.training_model is not None, 'Must compile the model ' \
                                                'before fitting data'
        y = np.zeros(shape=(x[0].shape[0],))  # doesn't get used
        return self.training_model.fit(x, y, **kwargs)
    
    def predict(self, x):
        assert self.prediction_model is not None and isinstance(
            self.prediction_model, Model)
        return self.prediction_model.predict_on_batch(x)
    
    def save_model(self, file_name, **kwargs):
        assert self.prediction_model is not None, 'Must compile the model ' \
                                                  'before ' \
                                                  'saving model'
...
class ConvolutionModel(LanguageModel):
    ...
    def build(self):
        ...
        cnns = [Convolution1D(filter_length=filter_length,
                              nb_filter=self.model_params.get('nb_filters',
                                                              1000),
                              activation=self.model_params.get(
                                  'conv_activation', 'relu'),
                              # W_regularizer=regularizers.l1(1e-4),
                              # activity_regularizer=regularizers.activity_l1(1e-4),
                              border_mode='same') for filter_length in
                [2, 3, 5, 7]]
        question_cnn = merge([cnn(question_dense) for cnn in cnns],
                             mode='concat')
...
class AttentionModel(LanguageModel):
    def build(self):
        ...
        f_rnn = LSTM(self.model_params.get('n_lstm_dims', 141),
                     return_sequences=True, dropout_U=0.2,
                     consume_less='mem')
        ...
        f_rnn = AttentionLSTM(self.model_params.get('n_lstm_dims', 141),
                              question_pool, return_sequences=True,
        ...
        answer_pool = merge([maxpool(answer_f_rnn), maxpool(answer_b_rnn)],
                            mode='concat', concat_axis=-1)
