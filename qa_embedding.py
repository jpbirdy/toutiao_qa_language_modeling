...
setences.extend(
        [list(np.array(x, dtype=str)) for x in question_info['words_seq'] if
         len(x) > 0])
setences.extend(
        [list(np.array(x, dtype=str)) for x in user_info['user_desc_words_sec']
         if len(x) > 0])
...
setences = []
setences.extend(
        [list(np.array(x, dtype=str)) for x in question_info['character_seq'] if
         len(x) > 0])
setences.extend([list(np.array(x, dtype=str)) for x in
                 user_info['user_desc_characters_sec'] if len(x) > 0])
