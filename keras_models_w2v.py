...
class LanguageModelW2V:
  def __init__(self, config):
    self.question = Input(shape=(config['question_len'], config['w2v_len']),
                          dtype='float32',
                          name='question_base')
    self.answer_good = Input(shape=(config['answer_len'], config['w2v_len']),
                             dtype='float32',
                             name='answer_good_base')
    self.answer_bad = Input(shape=(config['answer_len'], config['w2v_len']),
                            dtype='float32',
...
class AttentionModelW2V(LanguageModelW2V):
  def build(self):
    ...
    question_f_rnn = f_rnn(question)
    question_b_rnn = b_rnn(question)
    ...
    answer_f_rnn = f_rnn(answer)
    answer_b_rnn = b_rnn(answer)
